...
const parseSampleIntoGroup = ({ sample, groups, tolerance = 0.5 }: GroupingSampleParserParams) => {
  console.log(
    `Parsing sample with pace: ${sample.averagePace.quantity} ${sample.averagePace.unit}`,
  );

  const nearestMinutePace = Math.round(sample.averagePace.quantity);
  const isCloseEnough = Math.abs(sample.averagePace.quantity - nearestMinutePace) <= tolerance;

  if (!isCloseEnough) {
    console.warn(
      `Run with pace ${sample.averagePace.quantity} min/mile is not close enough to a whole minute. Skipping.`,
    );
  ...
  if (!groups[nearestMinutePace]) {
    groups[nearestMinutePace] = {
  ...
  const group = groups[nearestMinutePace];
